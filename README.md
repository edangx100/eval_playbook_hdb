...
4. **Guarantee coverage.** Two safety checks run after the LLM responds: every component that had failures is guaranteed at least one entry, and at least one entry must reference a concrete field from the agent's trace — so the taxonomy always ties back to something you can actually inspect.
